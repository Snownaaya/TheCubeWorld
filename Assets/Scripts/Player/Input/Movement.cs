...
public class Movement
{
    ...
    public void MovementUpdate()
    {
        ...
        {
            float rotation = Mathf.Atan2(_moveDirection.x, _moveDirection.y) * Mathf.Rad2Deg + _rotationY;
    ...
    public void OnFingerMove(Finger move)
    {
        if (move == _movementFinger)
        {
            ...
            if (_moveDirection.sqrMagnitude >= 0)
                _moveble.CharacterView.StartWalk();
            else
                _moveble.CharacterView.StopIdle();
        }
    }
    ...
    public void OnFingerUp(Finger finger)
    {
        ...
        if (_moveDirection.sqrMagnitude < 0)
            _moveble.CharacterView.StartIdle();
        else
            _moveble.CharacterView.StopWalk();
    }
}